...
# @raycast.title Google Calendar Focus
# @raycast.mode silent
...
if pgrep -x "Comet" > /dev/null; then
    # Comet is running, focus Google Calendar Focus tab or open new tab
...
tell application "Comet"
    ...
    if currentTabURL is not "" then
        if currentTabURL contains "calendar.google.com" then
    ...
    if foundTab then
        ...
        set newTab to make new tab at end of tabs of frontWindow with properties {URL:"https://calendar.google.com/calendar/u/0/r"}
...
APPLESCRIPT_EOF
    ...
    open -a Comet "https://calendar.google.com/calendar/u/0/r"
fi
