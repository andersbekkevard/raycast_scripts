...
if pgrep -x "Comet" > /dev/null; then
    # Comet is running, focus Toggl Focus tab or open new tab
...
tell application "Comet"
    ...
    if currentTabURL is not "" then
        if currentTabURL contains "track.toggl.com" or currentTabURL contains "toggl.com" then
    ...
    repeat with w from 1 to (count of windows)
        ...
        repeat with t from 1 to (count of windowTabs)
            try
                ...
                if tabURL contains "track.toggl.com" or tabURL contains "toggl.com" then
    ...
    if foundTab then
        ...
        set newTab to make new tab at end of tabs of frontWindow with properties {URL:"https://track.toggl.com/timer"}
...
APPLESCRIPT_EOF
    ...
    open -a Comet "https://track.toggl.com/timer"
fi
