...
# @raycast.title RISC-V Reference Card Focus
# @raycast.mode silent
...
if pgrep -x "Comet" > /dev/null; then
    # Comet is running, focus RISC-V Reference Card Focus tab or open new tab
...
tell application "Comet"
    ...
    if currentTabURL is not "" then
        if currentTabURL contains "RISC-V-Reference" then
    ...
    repeat with w from 1 to (count of windows)
        ...
        repeat with t from 1 to (count of windowTabs)
            try
                ...
                if tabURL contains "RISC-V-Reference" then
    ...
    if foundTab then
        ...
        set newTab to make new tab at end of tabs of frontWindow with properties {URL:"file:///Users/andersbekkevard/Library/CloudStorage/OneDrive-NTNU/Fag/Datamaskiner/RISC-V-Reference.pdf"}
