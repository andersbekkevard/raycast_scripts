...
# @raycast.title Formelark Fysikk Focus
# @raycast.mode silent
...
if pgrep -x "Comet" > /dev/null; then
    # Comet is running, focus Formelark Fysikk Focus tab or open new tab
...
tell application "Comet"
    ...
    if currentTabURL is not "" then
        if currentTabURL contains "Formelark%20Fysikk" then
    ...
    repeat with w from 1 to (count of windows)
        ...
        repeat with t from 1 to (count of windowTabs)
            try
                ...
                if tabURL contains "Formelark%20Fysikk" then
    ...
    if foundTab then
        ...
        set newTab to make new tab at end of tabs of frontWindow with properties {URL:"file:///Users/andersbekkevard/Library/CloudStorage/OneDrive-NTNU/Fag/Fysikk/Formelark%20Fysikk.pdf"}
