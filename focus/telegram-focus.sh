...
get_most_recent_url() {
    ...
    most_recent_url=$(sqlite3 "$temp_history"         "SELECT url FROM urls 
         WHERE (url LIKE '%web.telegram.org%')
...
if pgrep -x "Comet" > /dev/null; then
    # Comet is running, focus Telegram Focus tab or open new tab
...
tell application "Comet"
    ...
    if currentTabURL is not "" then
        if currentTabURL contains "web.telegram.org" then
    ...
    repeat with w from 1 to (count of windows)
        ...
        repeat with t from 1 to (count of windowTabs)
            try
                ...
                if tabURL contains "web.telegram.org" then
