...
# @raycast.title Pensumhefte AlgDat Focus
# @raycast.mode silent
...
if pgrep -x "Comet" > /dev/null; then
    # Comet is running, focus Pensumhefte AlgDat Focus tab or open new tab
...
tell application "Comet"
    ...
    if currentTabURL is not "" then
        if currentTabURL contains "pensumhefte.pdf" then
    ...
    repeat with w from 1 to (count of windows)
        ...
        repeat with t from 1 to (count of windowTabs)
            try
                ...
                if tabURL contains "pensumhefte.pdf" then
